...
**Recognising a procedure by its spelling is listed, or it breaks relinking** — `patina_core::by_spelling` names the procedures some part of Patina still decides on from the *name* a call was written with: `call-with-values` and `dynamic-wind`, in the VM's code generator. The desugarer renames a library macro's *references* to imported procedures where it emits them (#438, `Desugarer::early_bound`) and leaves those alone (`is_recognized`), because renaming one changes what the program does (#442). A new recogniser uses a constant from that module rather than a string literal; one that learns to ask about the binding takes its name out, as `apply` did (#443) and `call/cc` (#441).
