...
;; with its own top level, and ten of them defined `p`. Here they share one, so
...
;; non-promise. Gauche runs all 26.
;;
...
;; The delayed expression must not run until it is forced. `p-unforced` is
;; never forced anywhere in this file — that is the assertion.
...
(test-equal "the promise's thunk runs once, whatever it calls" 1
  (begin (force p-increment) (force p-increment) (force p-increment)
         increment-count))
...
;; twin below are the two chibi reports as failures (24 of 26 there; Gauche is
;; 26 of 26).
;;
...
;; R7RS §6.10: `make-promise` on a promise returns that promise.
...
;; forces the promise its body produces, without growing the recursion.
...
(define p-nested-outer (delay (+ 1 2)))
(define p-nested-inner (delay (force p-nested-outer)))
(test-equal "a promise whose body forces another promise" 3 (force p-nested-inner))
...
;; A lazy filter, which is the shape that needs `force` in tail position: when
;; the head fails the predicate the promise's value *is* the rest of the filter.
