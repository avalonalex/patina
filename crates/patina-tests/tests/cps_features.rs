//! CPS-specific feature tests — what could not migrate to Scheme.
//!
//! Most of this file is now `tests/scheme/control/cps-features.scm` (#193
//! Phase 1): 43 rows of plain R7RS control flow — exception handler stacks,
//! nested `dynamic-wind`, continuation capture, `guard`, and the routing of
//! runtime errors — run on both backends by `scheme_suite.rs` and arbitrated
//! by chibi and Gauche.
//!
//! What is left is what a portable `.scm` file cannot be:
//!
//! - **Delimited continuations.** `make-continuation-prompt-tag`,
//!   `call-with-continuation-prompt` and `abort-current-continuation` are
//!   measured absent from both chibi and Gauche, so these rows can never have
//!   an oracle. They are migrable in principle and are expected to follow, but
