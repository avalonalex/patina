...
///
/// NOTE: This function is now primarily used for testing. In production,
/// hygiene is applied during expansion in the Expander.
///
...
/// - `expansion_env`: The runtime environment to check for macro bindings
///
/// # Example
/// ```ignore
...
/// let hygienic = apply_hygiene(&expanded, &pattern_vars, &expansion_env);
/// // Result: (let ((##x#0 1)) ...)
/// ```
pub fn apply_hygiene(
    ...
    let free_identifiers = find_free_identifiers(expr, pattern_vars, expansion_env);
...
/// # Example
/// ```
/// use patina_macros::macro_expander::hygiene::gensym;
/// use std::rc::Rc;
///
...
fn find_free_identifiers(
    ...
    collect_free_identifiers(expr, pattern_vars, expansion_env, &mut free_ids);
    free_ids
}
...
fn is_macro(name: &str, expansion_env: &std::rc::Rc<patina_runtime::Environment>) -> bool {
    matches!(expansion_env.get(name), Some(Value::Macro { .. }))
}
    ...
    match expr {
        Value::Symbol(name) => {
            ...
            if !pattern_vars.contains(name)
                ...
                && !is_macro(name.as_ref(), expansion_env)
            {
        ...
        Value::Pair(pair) => {
            ...
            collect_free_identifiers(&borrowed.0, pattern_vars, expansion_env, free_ids);
            collect_free_identifiers(&borrowed.1, pattern_vars, expansion_env, free_ids);
        }
        Value::Vector(vec) => {
            for item in vec.borrow().iter() {
                collect_free_identifiers(item, pattern_vars, expansion_env, free_ids);
            }
        }
...
mod tests {
    ...
    fn empty_env() -> std::rc::Rc<patina_runtime::Environment> {
        std::rc::Rc::new(patina_runtime::Environment::new())
    }
