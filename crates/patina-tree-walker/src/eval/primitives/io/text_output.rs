...
pub(super) fn display(eval: &Evaluator, args: Vec<TaggedValue>) -> Result<TaggedValue, EvalError> {
...
/// Handles circular structures using datum labels (#n= and #n#)
pub(super) fn write(eval: &Evaluator, args: Vec<TaggedValue>) -> Result<TaggedValue, EvalError> {
    if args.is_empty() || args.len() > 2 {
        return Err(EvalError::WrongArity {
...
pub(super) fn newline(eval: &Evaluator, args: Vec<TaggedValue>) -> Result<TaggedValue, EvalError> {
    if args.len() > 1 {
        return Err(EvalError::WrongArity {
