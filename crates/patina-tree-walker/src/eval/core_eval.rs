...
fn eval_core_step(
    ...
    match expr {
        ...
        CoreExpr::CaseLambda { clauses } => {
            ...
            Ok(CoreEvalResult::Value(Value::Procedure(
...
fn core_expr_to_value(expr: &CoreExpr) -> Result<Value, EvalError> {
    match expr {
        ...
        CoreExpr::Expand { expr } => {
            ...
            Ok(cons(
                Value::Symbol(Rc::from("expand")),
                cons(expr_val, Value::Null),
            ))
        }
        CoreExpr::CaseLambda { clauses } => {
            // (case-lambda (params body...) ...)
            let clause_values: Result<Vec<Value>, EvalError> = clauses
                .iter()
                .map(|clause| {
                    let params_value = formals_to_value_list(&clause.params);
                    let body_values: Result<Vec<Value>, _> =
                        clause.body.iter().map(core_expr_to_value).collect();
                    let body_list = vec_to_list(&body_values?);

                    Ok(cons(params_value, body_list))
                })
                .collect();

            let clauses_list = vec_to_list(&clause_values?);
            Ok(cons(Value::Symbol(Rc::from("case-lambda")), clauses_list))
        }
