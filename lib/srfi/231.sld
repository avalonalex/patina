(define-library (srfi 231)
  (import (scheme base)
          ...
          (srfi 160 base)
          (srfi 231 base)
          ;; PATINA LOCAL EDIT: was `(chibi assert)`, which needs
          ;; `(chibi)` — implementation-specific, and external by the
          ;; bundling policy. All 55 uses across this library are the
          ;; single-expression form `(assert expr)`, and SRFI 145's `assume`
          ;; is exactly that: `lib/srfi/145.sld` takes the SRFI's *reporting*
          ;; sample, so a violated assumption raises rather than being
          ;; discarded. Verified to keep every domain check, including that a
          ;; bad interval is still refused. Recorded in
          ;; lib/srfi/PROVENANCE.md.
          (rename (srfi 145) (assume assert))
          ;; PATINA LOCAL EDIT: supplies the `u1vector-*` that
          ;; `231/transforms.scm`'s `u1-storage-class` is built from; see the
          ;; note there.
          (patina bitvector))
  (export
