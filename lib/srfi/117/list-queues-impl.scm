...
;; PATINA LOCAL EDIT: upstream is
;;   (define (list-queue-join! queue1 queue2)
;;     (set-cdr! (get-last queue1) (get-first queue2)))
;; which errors when queue1 is empty — `get-last` is then '() and `set-cdr!`
;; wants a pair, so `(list-queue-append! (list-queue) q)` raises — and which
;; leaves queue1's last pointer aimed at its own former last, so the next
;; `list-queue-add-back!` appends in the middle and `list-queue-back` answers
;; the wrong element. Larceny's suite raises on the first; the second is
;; unexercised by either suite. Guarding and re-pointing fixes both without
;; changing the joined result.
(define (list-queue-join! queue1 queue2)
  (cond
    ((list-queue-empty? queue2))
    ((list-queue-empty? queue1)
     (set-first! queue1 (get-first queue2))
