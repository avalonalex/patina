...
;;;
;;; The body of the SRFI's own reference implementation (David Van Horn, MIT),
;;; ported from R6RS to R7RS; see `PROVENANCE.md`. It is the only bundle that
;;; needed porting rather than copying — upstream ships `.sls` libraries, and
;;; the R6RS/R7RS differences it uses are small and marked in place.

          
  
  ;; PATINA LOCAL EDIT: R7RS record syntax. R6RS's `(fields a b)` names the
  ;; constructor, predicate and accessors implicitly; R7RS spells them out.
  ;; The generated names are the same ones the body already calls.
