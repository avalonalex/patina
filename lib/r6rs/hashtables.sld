(define-library (r6rs hashtables)
  ...
  (cond-expand
   ...
   ;;; PATINA DEVIATION. Upstream writes this guard
   ;;;
   ;;;     ((library (rnrs hashtables)))    ; nothing to do
   ;;;
   ;;; where the other two cond-expands in this same file — at the `(cond-expand`
   ;;; below and again further down — write
   ;;;
   ;;;     ((and (library (rnrs hashtables))
   ;;;           (not (library (r6rs no-rnrs)))))
   ;;;
   ;;; and so does every other guard in this tree. The `no-rnrs` marker exists
   ;;; to say "the `(rnrs ...)` you can see is Patina's re-export of *these*
   ;;; libraries, not a host R6RS implementation"; without it here, the presence
   ;;; of `lib/rnrs/hashtables.sld` takes this branch, `inexact-hash` and the
   ;;; other helpers below are never defined, and every non-fixnum key fails at
   ;;; the *caller* with `unbound variable: inexact-hash`. Measured: with the
   ;;; shim absent a float, rational and symbol key all work; with it present,
   ;;; none do. Restoring the missing conjunct is the whole edit.
   ((and (library (rnrs hashtables))
         (not (library (r6rs no-rnrs)))))    ; nothing to do
