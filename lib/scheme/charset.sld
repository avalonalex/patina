;; (scheme charset) - R7RS-large Red Edition
;;
;; Character sets.
;;
;; R7RS-large names this library `(scheme charset)`; it is SRFI 14 under its
;; standard-track name. This is a pure re-export of `(srfi 14)` -- the
...
(define-library (scheme charset)
  ...
  (export
    ...
    string->char-set! char-set-filter char-set-filter! ucs-range->char-set
    ->char-set ucs-range->char-set! char-set->list char-set->string
    ...
    char-set-difference! char-set-xor! char-set-diff+intersection!
    char-set:lower-case char-set:upper-case char-set:title-case char-set:letter
