...
| ✅ Unicode case: `(char-upcase #\ß)` ⇒ `#\S`; `char-ci=?` on final sigma; `string-ci=?` full folding — *fixed 2026-08-24* with the simple mappings. Family 7's numeric half *fixed 2026-09-12*: `char-numeric?` and `digit-value` share the Unicode 15.0 decimal table, including the omitted Kawi block. Both backends match all 680 decimal digits and their values in a full scalar sweep. VM `char` is now 139/139; the tree-walker's separate family 6 stack overflow remains | char, r6rs unicode | both |
