...
| Exact complex `number->string` rejects radices 2/8/16 — family 43. Correcting `exact?` enabled eight Larceny round-trip assertions; two pass, six expose this existing formatting restriction. Focused `complex` is now 62/69; `(log 0)` is its other existing failure | complex (6) | both |
