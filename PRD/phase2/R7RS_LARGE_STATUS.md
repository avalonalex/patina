...
| SRFI 101 | `(scheme rlist)` | Random-access lists | ✅ shipped | ✅ shipped (r-prefixed, 2026-08-26) |
...
Two of those did not arrive by bundling Scheme. SRFI 124 is implemented in Rust, because an ephemeron's defining property is a statement about what the collector does and there is no Scheme to vendor. SRFI 101 was ported from R6RS, because upstream ships `.sls` libraries and no R7RS version exists — and note its alias renames rather than re-exports: R7RS-large's `(scheme rlist)` is `rcons`/`rcar`/`rlist?`, since SRFI 101's own names shadow `(scheme base)`.
