...
- Adaptive collection is **on by default** since stage 4c (before it, Off was
  the default while the trigger still had a standing cost — see §6.1 for the
  measurements that justified the flip). The mode table and its environment
  grammar live in `patina-core` (`GcMode::from_env`) because the variables
  are process-global and both backends must agree on them:
  | Mode | Selected by | Behavior |
  |------|-------------|----------|
  | On (default) | — | adaptive threshold above |
  | Off | `PATINA_GC=0` | collect only when `(gc)` has been called |
