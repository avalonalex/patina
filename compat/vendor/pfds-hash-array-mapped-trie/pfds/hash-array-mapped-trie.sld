...
(define-library 
  ...
  (begin
    ...
    (define-record-type <subtrie>
                        (%make-subtrie size bitmap vector)
                        subtrie?
                        (size subtrie-size)
                        (bitmap subtrie-bitmap)
                        (vector subtrie-vector))
    ...
    (define-record-type <collision> 
                        (%make-collision size hash alist)
                        collision?
                        (size collision-size)
                        (hash collision-hash)
                        (alist collision-alist))
...
;;> returns the hash procedure used internally by the hamt.
    (define-record-type <hamt>
                        ...
                        (size hamt-size)
                        (root hamt-root)
                        (hash-function hamt-hash-function)
                        (equivalence-predicate hamt-equivalence-predicate))
      ...
      (case-lambda
        ((hamt key)
         ...
         (when (eqv? token return-val)
           (error 'hamt-ref "Key is not in the hamt" key))
         return-val)
        ((hamt key default)
         ;; assert hamt?
