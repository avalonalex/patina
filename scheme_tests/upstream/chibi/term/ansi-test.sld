(define-library (chibi term ansi-test)
  ...
  (import (scheme base)
          (scheme write)
          (chibi term ansi)
          ;; Patina adaptation: this suite has no chibi branch to resolve —
          ;; it always inlines a framework shim "to avoid circular
          ;; dependencies in snow installations", a packaging concern that
          ;; does not apply here, and the shim neither counts failures nor
          ;; reports through current-test-reporter, which the harness needs.
          ;; The shim's five definitions (test, test-assert, test-error,
          ;; test-begin, test-end) are replaced by the real (chibi test);
          ;; the two suite-specific helper macros below and every test body
          ;; are untouched. See scheme_tests/upstream/README.md.
          (chibi test))
  (begin
