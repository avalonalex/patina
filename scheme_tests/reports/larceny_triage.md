...
| Family 47, generated macros across a library | ✅ Fixed 2026-09-18 (#402): the matrix's second table, 45 shapes, reads 45 of 45 on both backends, 18 of them moved. The relinker's value-comparing skip the same review found (#407) was fixed 2026-09-19 with an equal-values axis: 63 of 63. The neighbouring shape — a generator that also *introduces* the definition its macro mentions, beside a plain one of that spelling or a second run's — was fixed the same day (#408): 40 more rows, 12 of them red on both backends and 10 on the VM alone, 103 of 103. A definition over an import after the expansion (#438) was fixed the same day too, 6 more rows red on both backends: 109 of 109 | None in this family. Family 40's *refusal* rows (#427) are separate; five procedures recognised by spelling are outside #438's fix (`patina_core::by_spelling`) |
...
table now (`MADE`, 109 shapes, scored against chibi and Gauche), 45 added for
family 47, 18 for #407's equal-values axis, 40 for #408's introducing generators and 6 for #438's defined-over import. What is left, in rough order of expected value: expansion nested
more than two levels; ellipsis depth greater than one; `define-record-type` and other derived binding
forms; literals in the pattern; the *refusal* half of a macro-introduced global as the binder
(family 40's axis; the half where the reference should reach it is `MADE`'s since #408 — its three refusal shapes live in
...
- Tracked as #422, a decision for the owner.
...
- Tracked as #418, a decision for the owner.
...
- **One check was added, and it was measured rather than assumed.** A written symbol is compared under the macro's definition scopes; an inherited identifier is compared under *its own*, which always include the generator's expansion scope — and that scope can select a binding the name alone does not reach: a definition the same expansion introduced beside the macro. The alias can only point at what the name reaches, so where the two views disagree the reference is left alone. With the check removed, a generator that introduces `(define X 'introduced)` beside a plain `(define X 'plain)` answers `plain` on both backends where chibi and Gauche answer `introduced`; with it, the shape stayed the loud `unbound` it was before this change, pinned in the matrix file. That shape is family 40's axis — a macro-introduced global as the binder — across a library boundary; without the plain sibling it answers `introduced`, correctly. **Fixed 2026-09-19 (#408):** where the two views disagree, the alias now names the *binding* the mention's scopes select — `Environment::introduced_definition` finds it in the library's root, as the global the VM renamed it to or the scope set the tree-walker filed it under, and the tree-walker's is reached through an alias that carries those scopes (`define_scoped_alias`). A lexical binding is still left alone. The matrix scores it as the `Introducing` makers, 40 rows measured under chibi and Gauche, 12 red on both backends — 8 loud, and the 4 with a top-level `X` at the use site silent, reading and assigning the program's. **Review found a VM-only half, fixed with it:** where one top-level form runs the definer twice — a macro expanding to two definer calls — the VM kept one record per *spelling* per form of the definitions it renamed (`RenameEnv::global_aliases` was a map by bare name), so the first run's identity was never recorded, and its macro was relinked by name to the second run's state: `(other use)` silently, in one program as well as across a library, 10 rows. It is a list now, an entry per definition. Two runs in two forms never showed it. The pin became `a_generated_macro_reaches_its_generators_definition_past_a_plain_one`. Traced over every suite file, the corpus on both backends and the three Larceny lanes, the new path is taken by nothing but the rows written for it, so no existing program's answer moved; expansion cost is within 2% of `main`.
...
- **Found by the same review, not new, and fixed 2026-09-19 (#407, after its root #406 made an import the library's location rather than a copy of its value):** the relinker's *skip* now asks whether the use site and the definition site reach one *location* by the name (`Environment::binding_location`). The matrix's second table gained the axis that had hidden it — the two `X`s holding equal values while the macro is expanded, 18 rows, 6 of them red on both backends — and reads 63 of 63. Traced over every suite file, the compat corpus and both Larceny lanes (some 34,000 decisions), the new question never decides differently from the old one, so no expansion gained an alias; it differs only where two bindings hold equal values. As it stood: the skip — nothing is aliased where the use site's binding of the name holds the same value as the definition site's — was a value comparison too, and had no binding to ask about, since an import copied values. A hand-written library macro `(set! X (+ X 1))` over a private `(define X 0)`, used from a program with its own `(define X 0)`, bumped the **program's** `X`: `(0 1)` for `(list (get-X) X)` on both backends, where chibi and Gauche answer `(1 0)`, as Patina now does. **#408's, and fixed with it the same day:** the pinned refusal above was loud only where the importing program had no binding of the name; where it had one the unlinked reference landed there (`use`), as it did before #402 and still did after #407 — that reference was left alone because its scopes select a binding the name alone does not reach, not because of any comparison of values.
- **What #407's skip still left, fixed 2026-09-19 (#438):** where the two sides *do* reach one binding the reference was left as a bare global name, which is looked up when it runs — so a program that defined the spelling itself **after** the expansion captured it. `define` over an import rightly gives the importer a binding of its own, in the same slot, and the template's reference followed the name there: one procedure, compiled once, bumped the library's `count` before the program's `(define count 100)` and the program's after it, and a library template's `(car x)` called a program's later `(define (car x) …)`. chibi and Gauche keep both the library's; they *disagree* about a reference the program itself wrote before its definition, and about a macro of the program's own, so neither of those is touched. Such a reference is now given a name of the use site's own for the import's **location** (`Environment::import_alias`): an ordinary forwarded slot, one per imported name, so it costs nothing at run time. Six `redefined` rows, red on both backends, written and generated macros alike. **Where the name is changed took two attempts, and the first is worth the record.** It was done in the expansion's syntax, beside relinking's other aliases, and review found five silent wrong answers against `main`: there an identifier is not yet known to be a reference, and it was a `case` datum (`((+) …)` stopped matching), a `let` binder capturing another expansion's alias, a `define` target landing on the shared slot, data handed to a quoting macro (`(car.1 cdr.3 list.0)`), and `cond-expand`'s `not`. Every lane had been green — no suite had an import-spelled name anywhere but in a call; `expansion/imported-names-in-templates.scm` has them now, fourteen rows. It is done where the desugarer *emits a reference or an assignment* (`Desugarer::early_bound`); relinking only records that the expansion is foreign. One decision cannot be made even there — the same expansion introducing a definition of the name, which is in no environment until it runs — and is taken back at the end of the top-level form; checked by mutation, with that step off exactly its two rows fail. **Four names are left out**, because some part of Patina recognises them by spelling *from the reference itself* and renaming one changes what the program does — measured, under both versions: the tree-walker lost every `call/cc` a library macro makes (the CPS transform claims it syntactically and the value is not callable there, Track Q §1.2, #441), and the VM lost one corpus package, five Larceny assertions and seven suite rows through `call-with-values` and `dynamic-wind` leaving the code generator's instruction path (#442). `apply` is listed with them and is *not* left out: its recogniser looks at the head of the form, so an `apply` passed as a value is bound like any other — it was excluded at first "so that the rule stays one rule", and a second review measured a template's `(map apply …)` calling a program's later `apply` where chibi and Gauche call the real one (#443 is what that recogniser still gets wrong). The list is `patina_core::by_spelling`, and the recognisers name its constants so it cannot drift. The same review found that **two names for one import must be two aliases** — one alias per location made the end-of-form step read back the wrong spelling, a regression against `main` — and two shapes this fix does not reach, each behaving as it did before: a reference inside a quasiquote's unquote, which is desugared by a separate pass after the form (#445 — the same pass leaves a `let-syntax` keyword unbound inside an unquote, on `main` too; fixed 2026-09-22, the desugarer desugars an unquoted expression in its form now), and a macro a library's generator defines in the program, used in a later top-level form (#446).
...
- **Spelling-based literal matching and `apply`-head check** (review of #114, pre-existing; since #438 `apply` is one of five names `patina_core::by_spelling` lists; #443): the literal matcher and the `apply` head test judge by spelling, so `(let ((else #f)) …)` around a macro using `my-cond`'s `else` literal, or a local `apply` around a template's `(apply f x)`, answer differently from chibi. The review of #132 added the import-level shape: a library's `(syntax-rules (quote) ((_ 'x) 'quoted) …)` under SRFI 101's `quote` still matches the literal by spelling, where chibi (comparing bindings) takes the other rule. Families 14/15 are fixed and this was not: locals became bindings for *resolution* while the literal matcher kept comparing spellings. **The local half closed 2026-09-13 (family 41)**: the matcher resolves both identifiers and compares the local bindings they reach. What remained was the global half — two names that reach no local binding still matched when spelled alike, so the SRFI 101 `quote` shape answered `quoted` before and after that change (measured). **That half closed 2026-09-22 (#450)** for two bound globals: they match only when they reach one location or hold one value, and the SRFI 101 shape answers `other`, as chibi and Gauche do. A name bound on one side and unbound on the other still matches (the oracles split on it). What remains is the `apply` head check, which is now `shadowed_names`' only reader.
