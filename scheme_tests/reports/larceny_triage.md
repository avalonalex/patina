...
| Family 47, generated macros across a library | ✅ Fixed 2026-09-18 (#402): the matrix's second table, 45 shapes, reads 45 of 45 on both backends, 18 of them moved. The relinker's value-comparing skip the same review found (#407) was fixed 2026-09-19 with an equal-values axis: 63 of 63 | One neighbouring shape pinned open: a generator that also *introduces* the definition its macro mentions, beside a plain one of that spelling — family 40's axis across a library |
...
table now (`MADE`, 63 shapes, scored against chibi and Gauche), 45 added for
family 47 and 18 for #407's equal-values axis. What is left, in rough order of expected value: expansion nested
...
- **Found by the same review, not new, and fixed 2026-09-19 (#407, after its root #406 made an import the library's location rather than a copy of its value):** the relinker's *skip* now asks whether the use site and the definition site reach one *location* by the name (`Environment::binding_location`). The matrix's second table gained the axis that had hidden it — the two `X`s holding equal values while the macro is expanded, 18 rows, 6 of them red on both backends — and reads 63 of 63. Traced over every suite file, the compat corpus and both Larceny lanes (some 34,000 decisions), the new question never decides differently from the old one, so no expansion gained an alias; it differs only where two bindings hold equal values. As it stood: the skip — nothing is aliased where the use site's binding of the name holds the same value as the definition site's — was a value comparison too, and had no binding to ask about, since an import copied values. A hand-written library macro `(set! X (+ X 1))` over a private `(define X 0)`, used from a program with its own `(define X 0)`, bumped the **program's** `X`: `(0 1)` for `(list (get-X) X)` on both backends, where chibi and Gauche answer `(1 0)`, as Patina now does. **Still open, and #408's:** the pinned refusal above is loud only where the importing program has no binding of the name; where it has one the unlinked reference lands there (`use`), as it did before #402 and still does after #407 — that reference is left alone because its scopes select a binding the name alone does not reach, not because of any comparison of values.
