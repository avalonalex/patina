...
- Ours: `crates/patina-tests/tests/scheme/expansion/hygiene.scm`'s row "a binder is scoped by where it stands", moved there by #193 Phase 1; `test_let_syntax_nested_lexical_scoping` (`crates/patina-tests/tests/let_syntax.rs`) pinned the same *answer* before it, and the suite row is where the reason now lives. What was new is why the answer is right. The defect was reproducible by enforcing Flatt's rule, which refused this program.
