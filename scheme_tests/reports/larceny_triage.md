...
- `chibi-tar` itself does **not** pass yet, and neither thing that stops it now is a hygiene defect (#404). It gets past `read-padded-string` and fails on `user-name`, which `chibi/tar.sld` imports from `(chibi system)` under `cond-expand (chibi …)` and defines a stand-in for only under `(chicken …)`; neither branch is Patina's. Behind that, `chibi-binary-record` writes strings and characters to a bytevector output port, which Patina refuses — while *reading* them from a bytevector input port, which it allows; chibi and Gauche allow both. With both worked around in a scratch copy the suite runs 15 of 15 on both backends, so those two are all that is left. The hygiene failure had been masking them.
