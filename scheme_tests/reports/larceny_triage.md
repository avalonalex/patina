...
- Ours: `crates/patina-tests/tests/scheme/data/numeric-operations.scm`, rows "sqrt is inexact even for exact squares" and "sqrt of a negative, and both branch cuts", which record the answers as they are. Both are registered against chibi and Gauche as `patina-defect` under issue #225 — the register is where the R7RS rule is now stated, and the rows fail when it is met.
