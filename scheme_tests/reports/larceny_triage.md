...
`crates/patina-tests/tests/larceny_families.rs`; #193 Phase 1 is moving them,
family by family, to the file about the defect's *subject* — mostly SRFI 64
suite files under `crates/patina-tests/tests/scheme/`, which run on chibi and
Gauche as well as on both backends. So an `- Ours:` line below names either a
Rust test function or a suite file plus the row names in it, and the file that
holds a family will change while this doc is alive.
