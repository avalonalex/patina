...
- **Both backends, confirmed.** It first looked tree-walker-only because the check could not see the VM: `alpha_rename` carried a second copy of the rule and never called the instrumented one. With the two copies merged into `patina_core::scope_resolve` (PR #135), `PATINA_AMBIGUITY_STRICT=1` refuses this program on the VM as well, at the same reference — so both backends make the same under-determined decision and are correct for the same extra-model reason.
...
- **The gate is one test wide.** `PATINA_AMBIGUITY_STRICT=1 cargo test --all --lib --tests --no-fail-fast` scores 95 of 96 binaries, failing only `test_let_syntax_nested_lexical_scoping` — this family. Nothing else in the repo resolves a reference the rule cannot decide, so the distance from here to a determined model is exactly this one shape.
