...
- That merge also showed how common the `TIE` case is. Measured over the same 249 processes, ties rose from 33 to **371** once the VM was included — 257 in the corpus, 48 in chibi-match on the VM, 33 in the R6RS lane — across a dozen identifiers (`ls`, `fk`, `s`, `i`, …). Two bindings of one name with the *same* scope set are ordinary, not a chibi-match quirk, and every one of them is decided by nesting order rather than by scopes.
