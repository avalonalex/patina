...
- **Attempted 2026-08-25 and backed out; what it established is worth more than the attempt.** The visible defect is not in `guard` alone, and fixing `guard` alone cannot fix it:
  - **The root cause is one level down.** `with-exception-handler` runs its handler *after* unwinding to the handler's own wind depth, so the handler never runs in the raise's dynamic environment as R7RS 6.11 requires. Measured on both backends: `(with-exception-handler (lambda (e) (log 'handler) 'handled) (lambda () (dynamic-wind in (lambda () (raise-continuable 'x)) out)))` logs `(in out handler out)` — note the after-thunk running *twice* — where chibi and Gauche log `(in handler out)`. Until that is fixed no `guard` can re-enter anything, because there is nothing left to re-enter by the time its handler runs. The VM's unwind loop is at the top of `vm_raise_value`.
  - **Re-entry itself already works**: a continuation captured inside a `dynamic-wind`, escaped from and re-invoked, runs the before-thunk on both backends (`(in out in out)`, matching both references). So the machinery is there; the raise path does not use it.
  - **R7RS 7.3's reference `guard`** (the double `call/cc` — `guard-k` out to the guard's environment for the clauses, `handler-k` back to the raise point to re-raise) was written and does converge the tree-walker divergence recorded in PRD §6 as "a `guard` handler runs before the unwind": both backends then give `(before after handler)` like chibi, and the VM's chibi suite stays 1226/1226. But **the tree-walker loses three assertions** (`6.11 Exceptions` runs 27 of 30, two reporting `unhandled continuable exception`): its handler stack does not survive the jump back, which is the same gap PRD §6 records for `CpsContinuation`, and swapping the reference's `raise-continuable` for `raise` does not help, so it is the structure and not that call. That gap wants the `MachineState` refactor §6 already describes.
  - Order for whoever takes it: the tree-walker's handler stack, then the raise path's unwind in both backends, then the reference `guard` — at which point this family and PRD §6's tree-walker `guard` entry close together.
