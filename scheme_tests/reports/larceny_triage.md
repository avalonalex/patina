...
- Ours: `crates/patina-tests/tests/scheme/expansion/let-syntax.scm`'s row "a template's quote is not captured by a use-site let-syntax", and `a_templates_quote_is_the_definition_sites_quote_under_an_imported_one` — the import half needs a library to have anything private to reach, so it stays in Rust until a suite file can hold it (chibi does not support `define-library` in a script).
