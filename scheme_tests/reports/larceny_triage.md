...
- **Still open, found by the same review and not new (#407; its root #406 was fixed 2026-09-19 — an import is the library's location now, not a copy of its value — which gives the skip below a binding to ask about, and the skip has not been changed yet):** the relinker's *skip* — nothing is aliased where the use site's binding of the name holds the same value as the definition site's — is a value comparison too, and has no binding to ask about, since an import copies values. A hand-written library macro `(set! X (+ X 1))` over a private `(define X 0)`, used from a program with its own `(define X 0)`, bumps the **program's** `X`: `(0 1)` for `(list (get-X) X)` on both backends, on `main` as well, where chibi and Gauche answer `(1 0)`. And the pinned refusal above is loud only where the importing program has no binding of the name; where it has one the unlinked reference lands there (`use`), as it did before #402.
