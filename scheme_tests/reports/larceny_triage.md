...
### 38. A scoped write cannot reach the binding its own read can — tree-walker only — ✅ fixed 2026-08-27, fallback halves 2026-08-28
...
- **The fallback halves were not aligned either — ✅ fixed 2026-08-28.** Measured on `main` before the fix:

```scheme
(define (g) (define c 5) (define-syntax b (syntax-rules () ((b) (set! c (+ c 10))))) (b) c)
(g)   ; VM and chibi: 15 — tree-walker: Undefined variable: c

(define c 'global)                      ; the same program with a global beside it
(list (g) c)   ; VM and chibi: (15 global) — tree-walker: (5 15)
```

- Three ways the write differed from the read, all now gone. It fell back at the **root** — by then the recursion had walked there, so `self.set` assigned the global — where reading falls back at the environment the lookup started in. It resolved **one environment at a time**, taking the largest candidate in the innermost environment holding any, where reading takes the largest in the whole chain. And it carried its own copy of the rule rather than calling `patina_core::scope_resolve`, so an ambiguous write picked by size where an ambiguous read was refused.
- The second program is why this was worth doing ahead of family 36. It is not an error but a **silent corruption**: the local stays 5, and an unrelated global two frames away is overwritten by a macro expanded inside `g`. Nothing in the program mentions `c` being global.
- The fix is that `set_with_scopes` is now the mirror image of `get_with_scopes` — same walk, same order, same `resolve_scoped`, same by-name fallback from the same starting environment — collecting a *location* where the read collects a value. Pinned by `an_introduced_assignment_falls_back_in_its_own_frame` and `an_introduced_assignment_does_not_escape_its_frame_and_clobber_a_global`; both fail if the root fallback is put back.
- Still hand-rolled, and deliberately: `has_scoped_binding` tests subset with no most-specific rule, because its one caller asks only whether *some* scoped binding is visible and never which.
